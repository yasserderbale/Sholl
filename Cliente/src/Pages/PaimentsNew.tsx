import { useState, useEffect, useMemo } from 'react';
import {
  Box,
  Typography,
  Button,
  Checkbox,
  Chip,
  IconButton,
  Modal,
  TextField,
  Table,
  TableBody,
  TableCell,
  TableContainer,
  TableHead,
  TableRow,
  Paper,
  Select,
  MenuItem,
  FormControl,
  InputLabel,
  Snackbar,
  Alert,
  InputAdornment
} from '@mui/material';
import {
  Add,
  Search,
  Close as CloseIcon,
  Edit,
  Delete,
  Visibility,
  ArrowBack,
  Receipt
} from '@mui/icons-material';
import { usAuth } from '../Context/AuthContext';

// Types
type Student = {
  id: string;
  Name: string;
  modules?: string[]; // المواد التي يدرسها التلميذ
};

type Group = {
  id: string;
  name: string;
  Studentid: string[];
};

type Subject = {
  id: string;
  name: string;
  price: number; // السعر مخفي في الواجهة
};

type PaymentRecord = {
  id: string;
  studentId: string[];
  subjectId: string;
  month: string;
  year: string;
  amount: number; // مخفي في الواجهة
  date: string;
  method: string;
  status: 'paid' | 'pending' | 'overdue';
  invoiceNumber?: string;
};

export function Paimentes() {
  const { groupe, stude, tocken, mat } = usAuth() as {
    groupe: Group[];
    stude: Student[];
    tocken: string;
    mat: any[]; // المواد مع الأسعار من الـ Context
  };

  const [openModal, setOpenModal] = useState(false);
  const [selectedGroup, setSelectedGroup] = useState<string>('');
  const [students, setStudents] = useState<Student[]>([]);
  const [selectedStudent, setSelectedStudent] = useState<string>(''); // تلميذ واحد فقط
  const [selectedSubject, setSelectedSubject] = useState<string>('');
  const [selectedMonth, setSelectedMonth] = useState<string>('');
  const [selectedYear, setSelectedYear] = useState<string>('');
  const [paymentMethod, setPaymentMethod] = useState<string>('cash');
  const [searchTerm, setSearchTerm] = useState<string>(''); // متغير البحث
  const [viewMode, setViewMode] = useState<'list' | 'details'>('list'); // وضع العرض
  const [selectedPayment, setSelectedPayment] = useState<PaymentRecord | null>(null); // الدفعة المختارة
  const [studentPayments, setStudentPayments] = useState<PaymentRecord[]>([]); // مدفوعات التلميذ
  const [toast, setToast] = useState<{
    open: boolean;
    message: string;
    severity: 'success' | 'error' | 'info' | 'warning';
  }>({ open: false, message: '', severity: 'success' });

  // المواد من الـ Context مع الأسعار
  const subjects: Subject[] = useMemo(() => {
    return mat.map(matiere => ({
      id: matiere._id || matiere.id,
      name: matiere.name,
      price: matiere.prix
    }));
  }, [mat]);

  // الأشهر
  const months = [
    'يناير', 'فبراير', 'مارس', 'أبريل', 'مايو', 'يونيو',
    'يوليو', 'أغسطس', 'سبتمبر', 'أكتوبر', 'نوفمبر', 'ديسمبر'
  ];

  // التلاميذ مع موادهم من الـ Context
  const studentsWithModules = useMemo(() => {
    return stude.map(student => {
      console.log('Student data:', student); // للتحقق من بنية البيانات
      return {
        id: student._id || student.id,
        Name: student.Name,
        // محاولة استخراج المواد بطرق مختلفة
        modules: student.modules?.map((mod: any) => {
          // إذا كان mod هو معرف المادة مباشرة
          if (typeof mod === 'string') return mod;
          // إذا كان كائن يحتوي على matid
          if (mod.matid) return mod.matid._id || mod.matid;
          // إذا كان كائن يحتوي على _id
          return mod._id || mod.id;
        }) || subjects.map(s => s.id) // إذا لم توجد مواد، اعطه جميع المواد
      };
    });
  }, [stude, subjects]);

  // بيانات وهمية للاختبار (سيتم استبدالها ببيانات حقيقية)
  const paymentsData: PaymentRecord[] = [];

  // Filtrage
  const filteredGroups = useMemo(() => {
    if (!searchTerm) return groupe;
    return groupe.filter(g => g.name.toLowerCase().includes(searchTerm.toLowerCase()));
  }, [groupe, searchTerm]);

  // المواد التي يدرسها التلميذ المختار
  const studentSubjects = useMemo(() => {
    if (!selectedStudent) return [];
    const student = studentsWithModules.find(s => s.id === selectedStudent);
    if (!student || !student.modules) return [];
    return subjects.filter(subject => student.modules!.includes(subject.id));
  }, [selectedStudent, studentsWithModules, subjects]);

  // مدفوعات التلميذ المختار (من الباك إند)
  const currentStudentPayments = studentPayments;

  // حالة كل مادة للتلميذ المختار
  const getSubjectPaymentStatus = (subjectId: string, month: string) => {
    const payment = studentPayments.find((p: PaymentRecord) => 
      p.subjectId === subjectId && p.month === month
    );
    return payment?.status || 'pending';
  };

  // لون المادة حسب حالة الدفع
  const getSubjectColor = (subjectId: string, month: string) => {
    const status = getSubjectPaymentStatus(subjectId, month);
    switch (status) {
      case 'paid': return '#dcfce7'; // أخضر فاتح
      case 'pending': return '#fef3c7'; // أصفر فاتح
      case 'overdue': return '#fee2e2'; // أحمر فاتح
      default: return '#f3f4f6'; // رمادي
    }
  };

  const getSubjectBorderColor = (subjectId: string, month: string) => {
    const status = getSubjectPaymentStatus(subjectId, month);
    switch (status) {
      case 'paid': return '#22c55e'; // أخضر
      case 'pending': return '#f59e0b'; // أصفر
      case 'overdue': return '#ef4444'; // أحمر
      default: return '#d1d5db'; // رمادي
    }
  };

  // تحديث قائمة التلاميذ عند اختيار مجموعة
  useEffect(() => {
    if (selectedGroup) {
      const group = groupe.find(g => g.id === selectedGroup);
      if (group) {
        setStudents(studentsWithModules.filter(s => group.Studentid.includes(s.id)));
      } else {
        setStudents([]);
      }
    } else {
      setStudents([]);
    }
  }, [selectedGroup, groupe, studentsWithModules]);

  // تحديث مدفوعات التلميذ عند اختياره
  useEffect(() => {
    if (selectedStudent) {
      fetchStudentPayments(selectedStudent);
    } else {
      setStudentPayments([]);
    }
  }, [selectedStudent, tocken]);

  // تحميل مدفوعات التلميذ من الباك إند (بالفرنسية)
  const fetchStudentPayments = async (studentId: string) => {
    try {
      const response = await fetch(`http://localhost:3000/paiements/etudiant/${studentId}`, {
        headers: {
          'Authorization': `Bearer ${tocken}`
        }
      });
      const result = await response.json();
      if (response.ok) {
        // تحويل بيانات الباك إند إلى الشكل المطلوب
        const payments = result.data?.map((p: any) => ({
          id: p._id || p.id,
          studentId: [studentId],
          subjectId: p.matiereId || p.matiere?._id,
          month: p.mois || '',
          year: p.annee || '2025',
          amount: p.montant || 0,
          date: p.datePaiement || p.dateCreation || '',
          method: p.methodePaiement || 'نقداً',
          status: p.statut === 'paye' ? 'paid' : p.statut === 'en_retard' ? 'overdue' : 'pending',
          invoiceNumber: p.numeroFacture
        })) || [];
        setStudentPayments(payments);
      }
    } catch (error) {
      console.error('خطأ في تحميل المدفوعات:', error);
    }
  };

  // Functions
  const closeModal = () => {
    setOpenModal(false);
    setSelectedSubject('');
    setSelectedMonth('');
    setSelectedYear('');
    setPaymentMethod('cash');
  };

  const handleSavePayment = async () => {
    if (!selectedSubject || !selectedMonth || !selectedYear || !selectedStudent) {
      setToast({ open: true, message: 'يرجى ملء جميع الحقول المطلوبة', severity: 'error' });
      return;
    }

    // API call لحفظ الدفعة (بالفرنسية)
    try {
      const response = await fetch('http://localhost:3000/paiements', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${tocken}`,
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          etudiantId: selectedStudent,
          matiereId: selectedSubject,
          mois: selectedMonth,
          annee: selectedYear,
          montant: subjects.find(s => s.id === selectedSubject)?.price || 0,
          methodePaiement: paymentMethod,
          datePaiement: new Date().toISOString()
        })
      });
      
      const result = await response.json();
      if (response.ok) {
        setToast({ open: true, message: 'تم حفظ الدفعة بنجاح', severity: 'success' });
        closeModal();
        // إعادة تحميل مدفوعات التلميذ
        fetchStudentPayments(selectedStudent);
      } else {
        setToast({ open: true, message: result.message || 'خطأ في حفظ الدفعة', severity: 'error' });
      }
    } catch (error) {
      setToast({ open: true, message: 'خطأ في الاتصال بالخادم', severity: 'error' });
    }
  };

  const getStatusColor = (status: string) => {
    switch (status) {
      case 'paid': return 'success';
      case 'pending': return 'warning';
      case 'overdue': return 'error';
      default: return 'default';
    }
  };

  const getStatusText = (status: string) => {
    switch (status) {
      case 'paid': return 'مدفوع';
      case 'pending': return 'معلق';
      case 'overdue': return 'متأخر';
      default: return status;
    }
  };

  // واجهة اختيار التلميذ وعرض مواده
  const renderStudentSelection = () => {

    return (
      <>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
          <Typography variant="h4">💰 إدارة المدفوعات</Typography>
        </Box>

        {/* اختيار المجموعة والتلميذ */}
        <Box sx={{ mb: 3, display: 'flex', gap: 2 }}>
          <FormControl fullWidth required>
            <InputLabel>اختيار المجموعة *</InputLabel>
            <Select
              value={selectedGroup}
              label="اختيار المجموعة *"
              onChange={e => {
                setSelectedGroup(e.target.value);
                setSelectedStudent('');
              }}
            >
              {filteredGroups.map(g => (
                <MenuItem key={g.id} value={g.id}>{g.name}</MenuItem>
              ))}
            </Select>
          </FormControl>
          
          <FormControl fullWidth required disabled={!selectedGroup}>
            <InputLabel>اختيار التلميذ *</InputLabel>
            <Select
              value={selectedStudent}
              label="اختيار التلميذ *"
              onChange={e => setSelectedStudent(e.target.value)}
            >
              {students.map(s => (
                <MenuItem key={s.id} value={s.id}>{s.Name}</MenuItem>
              ))}
            </Select>
          </FormControl>
        </Box>

        {/* عرض مواد التلميذ مع الأسعار والألوان */}
        {selectedStudent && (
          <Box mb={4}>
            <Typography variant="h6" gutterBottom>
              مواد {studentsWithModules.find(s => s.id === selectedStudent)?.Name} - شهر يناير 2025
            </Typography>
            
            {/* عرض معلومات التلميذ */}
            <Box mb={2} p={2} sx={{ backgroundColor: '#f5f5f5', borderRadius: 2 }}>
              <Typography variant="body2">
                <strong>عدد المواد:</strong> {studentSubjects.length} مادة
              </Typography>
              <Typography variant="body2">
                <strong>إجمالي المبلغ الشهري:</strong> {studentSubjects.reduce((sum, s) => sum + s.price, 0)} دج
              </Typography>
            </Box>
            
            <Box display="grid" gridTemplateColumns="repeat(auto-fill, minmax(300px, 1fr))" gap={2} mb={3}>
              {studentSubjects.map(subject => {
                const status = getSubjectPaymentStatus(subject.id, 'يناير');
                const payment = studentPayments.find(p => p.subjectId === subject.id && p.month === 'يناير');
                
                return (
                  <Paper 
                    key={subject.id}
                    sx={{
                      p: 3,
                      backgroundColor: getSubjectColor(subject.id, 'يناير'),
                      border: `2px solid ${getSubjectBorderColor(subject.id, 'يناير')}`,
                      cursor: 'pointer',
                      '&:hover': {
                        transform: 'translateY(-2px)',
                        boxShadow: 3
                      },
                      transition: 'all 0.2s'
                    }}
                    onClick={() => {
                      if (status === 'pending' || status === 'overdue') {
                        setSelectedSubject(subject.id);
                        setSelectedMonth('يناير');
                        setSelectedYear('2025');
                        setOpenModal(true);
                      }
                    }}
                  >
                    <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                      <Typography variant="h6" sx={{ fontWeight: 'bold' }}>
                        {subject.name}
                      </Typography>
                      <Chip 
                        label={getStatusText(status)} 
                        color={getStatusColor(status) as any}
                        size="small"
                      />
                    </Box>
                    
                    <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                      <Typography variant="h5" sx={{ fontWeight: 'bold', color: getSubjectBorderColor(subject.id, 'يناير') }}>
                        {subject.price} دج
                      </Typography>
                      {status === 'paid' && payment?.date && (
                        <Typography variant="body2" color="text.secondary">
                          تاريخ الدفع: {new Date(payment.date).toLocaleDateString('ar-SA')}
                        </Typography>
                      )}
                    </Box>
                    
                    {status === 'paid' && payment?.method && (
                      <Typography variant="body2" color="text.secondary">
                        طريقة الدفع: {payment.method}
                      </Typography>
                    )}
                    
                    {(status === 'pending' || status === 'overdue') && (
                      <Typography variant="body2" sx={{ mt: 1, fontStyle: 'italic', color: getSubjectBorderColor(subject.id, 'يناير') }}>
                        اضغط للدفع • {subject.price} دج
                      </Typography>
                    )}
                  </Paper>
                );
              })}
            </Box>
            
            {/* قائمة مدفوعات التلميذ */}
            <Typography variant="h6" gutterBottom sx={{ mt: 4 }}>
              تاريخ المدفوعات
            </Typography>
            <TableContainer component={Paper}>
              <Table>
                <TableHead>
                  <TableRow>
                    <TableCell>المادة</TableCell>
                    <TableCell>الشهر/السنة</TableCell>
                    <TableCell>المبلغ</TableCell>
                    <TableCell>تاريخ الدفع</TableCell>
                    <TableCell>الطريقة</TableCell>
                    <TableCell>الحالة</TableCell>
                    <TableCell align="center">الإجراءات</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {studentPayments.length === 0 ? (
                    <TableRow><TableCell colSpan={7} align="center">
                      لا توجد مدفوعات
                    </TableCell></TableRow>
                  ) : (
                    studentPayments.map(payment => {
                      const subject = subjects.find(s => s.id === payment.subjectId);
                      return (
                        <TableRow key={payment.id}>
                          <TableCell>{subject?.name || 'مادة غير معروفة'}</TableCell>
                          <TableCell>{payment.month} {payment.year}</TableCell>
                          <TableCell>{payment.amount} دج</TableCell>
                          <TableCell>
                            {payment.date ? new Date(payment.date).toLocaleDateString('ar-SA') : '-'}
                          </TableCell>
                          <TableCell>{payment.method || '-'}</TableCell>
                          <TableCell>
                            <Chip 
                              label={getStatusText(payment.status)} 
                              color={getStatusColor(payment.status) as any}
                              size="small"
                            />
                          </TableCell>
  };

  // Vue détails
  const renderDetailsView = () => {
    if (!selectedPayment) return null;
    const studentsList = stude.filter(s => selectedPayment.studentId.includes(s.id));
    
    // Get all payments for this student
    const studentId = selectedPayment.studentId[0];
    const allStudentPayments = studentPayments.filter((p: PaymentRecord) => p.studentId.includes(studentId));

    return (
      <Box>
        <Button startIcon={<ArrowBack />} onClick={() => setViewMode('list')} sx={{ mb: 2 }}>
          العودة للقائمة
        </Button>
        <Paper sx={{ p: 3, mb: 3 }}>
          <Typography variant="h5" gutterBottom>
            تاريخ المدفوعات - {studentsList[0]?.Name || 'تلميذ غير معروف'}
          </Typography>
          <Typography variant="body2" color="text.secondary" gutterBottom>
            المجموع: {allStudentPayments.length} دفعة
          </Typography>
        </Paper>

        <TableContainer component={Paper}>
          <Table>
            <TableHead>
              <TableRow>
                <TableCell>المادة</TableCell>
                <TableCell>الشهر/السنة</TableCell>
                <TableCell>التاريخ</TableCell>
                <TableCell>الطريقة</TableCell>
                <TableCell>الحالة</TableCell>
                <TableCell align="center">الإجراءات</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {allStudentPayments.map(payment => {
                const subject = subjects.find(s => s.id === payment.subjectId);
                return (
                  <TableRow key={payment.id}>
                    <TableCell>{subject?.name || 'مادة غير معروفة'}</TableCell>
                    <TableCell>{payment.month} {payment.year}</TableCell>
                    <TableCell>
                      {payment.date ? new Date(payment.date).toLocaleDateString('ar-SA') : '-'}
                    </TableCell>
                    <TableCell>{payment.method || '-'}</TableCell>
                    <TableCell>
                      <Chip 
                        label={getStatusText(payment.status)} 
                        color={getStatusColor(payment.status) as any}
                        size="small"
                      />
                    </TableCell>
                    <TableCell align="center">
                      <IconButton size="small" title="تعديل">
                        <Edit />
                      </IconButton>
                      <IconButton size="small" color="error" title="حذف">
                        <Delete />
                      </IconButton>
                      {payment.invoiceNumber && (
                        <IconButton size="small" title="عرض الفاتورة">
                          <Receipt />
                        </IconButton>
                      )}
                    </TableCell>
                  </TableRow>
                );
              })}
            </TableBody>
          </Table>
        </TableContainer>
      </Box>
    );
  };

  return (
    <Box p={3}>
      {viewMode === 'list' ? renderStudentSelection() : renderDetailsView()}

      {/* Modal إضافة دفعة */}
      <Modal open={openModal} onClose={closeModal}>
        <Box sx={{
          position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)',
          width: 500, bgcolor: 'background.paper', boxShadow: 24, p: 4, borderRadius: 2
        }}>
          <Box display="flex" justifyContent="space-between" mb={3}>
            <Typography variant="h6">تأكيد الدفعة</Typography>
            <IconButton onClick={closeModal}><CloseIcon /></IconButton>
          </Box>

          {selectedStudent && selectedSubject && (
            <Box mb={3}>
              <Typography variant="body1" gutterBottom>
                <strong>التلميذ:</strong> {stude.find(s => s.id === selectedStudent)?.Name}
              </Typography>
              <Typography variant="body1" gutterBottom>
                <strong>المادة:</strong> {subjects.find(s => s.id === selectedSubject)?.name}
              </Typography>
              <Typography variant="body1" gutterBottom>
                <strong>الشهر:</strong> {selectedMonth} {selectedYear}
              </Typography>
              <Typography variant="h6" sx={{ color: 'primary.main', mt: 2 }}>
                <strong>المبلغ:</strong> {subjects.find(s => s.id === selectedSubject)?.price} دج
              </Typography>
            </Box>
          )}


          <FormControl fullWidth sx={{ mb: 3 }}>
            <InputLabel>طريقة الدفع</InputLabel>
            <Select
              value={paymentMethod}
              label="طريقة الدفع"
              onChange={e => setPaymentMethod(e.target.value)}
            >
              <MenuItem value="cash">نقداً</MenuItem>
              <MenuItem value="bank">تحويل بنكي</MenuItem>
              <MenuItem value="check">شيك</MenuItem>
            </Select>
          </FormControl>

          <Box display="flex" justifyContent="flex-end" gap={2}>
            <Button onClick={closeModal}>إلغاء</Button>
            <Button variant="contained" onClick={handleSavePayment} color="success">
              تأكيد الدفع
            </Button>
          </Box>
        </Box>
      </Modal>


      <Snackbar open={toast.open} autoHideDuration={6000} onClose={() => setToast({ ...toast, open: false })}>
        <Alert onClose={() => setToast({ ...toast, open: false })} severity={toast.severity}>
          {toast.message}
        </Alert>
      </Snackbar>
    </Box>
  );
}
