  ...
  useEffect(() => {
    ...
    if (selectedGroup) {
      ...
      if (group) {
        ...
        console.log('Group Studentid:', group.Studentid);
        
        // استخراج التلاميذ بناء على Studentid
        let groupStudents = [];
        
        if (group.Studentid && Array.isArray(group.Studentid) && group.Studentid.length > 0) {
          // فلترة التلاميذ بناء على معرفاتهم في المجموعة
          groupStudents = stude?.filter(student => {
            const studentId = student.id || student._id;
            return group.Studentid.includes(studentId);
          }) || [];
        } else {
