...
export const RegistnewPaimente = async ({
  ...
  if (!checkpaiment) {
    ...
    const totalprcieMat = chechMatieres.reduce(
      (accumalitore, currentvalue) => (accumalitore += currentvalue.prix),
      0
    );
    ...
    const insertpaiment = await paimentesModel.create({
      idStud: idStud,
      paimentes: [
        { Mois, Montante, Date, matieres: mapMatieres, status: bool },
      ],
    });
  ...
  if (boolMountch.length > 0)
    return { StatusCode: 400, data: `Ce Mois deja Payees :${boolMountch}` };
  const totalprcieMat = chechMatieres.reduce(
    (accumalitore, currentvalue) => (accumalitore += currentvalue.prix),
    0
  );
...
export const getPaimentes = async ({ identifiante }: Igetpaimente) => {
  if (!identifiante) return { StatusCode: 404, data: "failed chek tocken " };
  const getpaimentes = await paimentesModel
    .find()
    .populate("idStud")
    .populate("paimentes.matieres.idMat");
...
export const getOnePaimente = async ({
  ...
  const getOneStudent = await paimentesModel
    .findOne({ idStud })
    .populate("idStud");
  if (!getOneStudent) return { StatusCode: 404, data: "failed check Student " };
  const getOnepaime = (getOneStudent.paimentes as any).filter(
    (item: any) => item._id == idPaiment
  );
...
export const SearchePaiementStud = async ({
  identifiante,
  search,
}: Isearch) => {
  if (!identifiante) return { StatusCode: 404, data: "failed check tocken " };
  const checkSearch = await paimentesModel
    .find()
    .populate({
      path: "idStud",
