  ...
  return (
    <Box className={Styles.dashboard} p={3}>
      ...
      >
        ...
        <Box sx={{ flex: '1 1 250px', minWidth: '250px' }}>
          <Paper className={Styles.card} elevation={3}>
            ...
            <Typography variant="h6">Nombre de paiements</Typography>
      ...
      <Box className={Styles.graphContainer} mt={4}>
        ...
        >
          <MonetizationOnIcon color="primary" /> {t('monthlyRevenue') || 'الإيرادات الشهرية'}
        </Typography>
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-end', height: '300px', padding: '20px', backgroundColor: '#f9fafb', borderRadius: '12px' }}>
          {stats?.monthlyRevenue?.map((item: any, index: number) => {
            const maxAmount = Math.max(...(stats?.monthlyRevenue?.map((r: any) => r.amount) || [1]));
