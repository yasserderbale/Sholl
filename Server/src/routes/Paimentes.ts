...
import { CompletePaymente, getOnePaimente, getPaimentes, RegistnewPaimente, SearchePaiementStud } from "../services/Paimentes"
...
const response = await CompletePaymente({identifiante,idStud,idPaiment,addPricee})
res.status(response.StatusCode).json(response)
})
